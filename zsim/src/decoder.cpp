...
uint64_t Decoder::lastOvecOperand = 0;
std::queue<uint64_t> Decoder::ovecLoadAddresses;
            ...
            switch (opcode) {
                ...
                case XO(XCHG):
                    ...
                    if (INS_IsXchg(ins) && INS_OperandReg(ins, 0) == REG_RCX && INS_OperandReg(ins, 1) == REG_RCX) {
                        uint32_t baseAddr = static_cast<uint32_t>(lastOvecOperand >> 32);
                        uint32_t orient = static_cast<uint32_t>(lastOvecOperand & 0xFFFFFFFF);
                        const int maxLoadNum = 64 / sizeof(float);
                        uint64_t lastCacheLine = 0;

                        for (int _i = 0; _i < maxLoadNum; _i++) {
                            uint64_t _addr = baseAddr + _i * orient;
                            uint64_t _blockAddr = _addr / zinfo->lineSize;
                            if (_blockAddr == lastCacheLine) continue;

                            ovecLoadAddresses.push(_addr);
                            lastCacheLine = _blockAddr;

                            DynUop uop;
                            uop.clear();
                            uop.rs[0] = 0;  // Assume there are a few specific registers for OVEC; doesn't matter much
                            uop.rs[1] = 0;
                            uop.portMask = PORT_2;
                            uop.type = UOP_OVEC;
                            uops.push_back(uop);
