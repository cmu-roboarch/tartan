...
class ReplPolicy : public GlobAlloc {
    ...
    public:
        ...
        virtual void manipulate(uint32_t id) {}
};
...
class FcpPolicy : public ReplPolicy {
    protected:
        uint32_t** array;
        uint32_t numLines;
        uint32_t numWays;
    ...
    public:
        explicit FcpPolicy(uint32_t _numLines, uint32_t _numWays) : numLines(_numLines), numWays(_numWays) {
            numSets = numLines / numWays;

            array = gm_malloc<uint32_t*>(numSets);
            for (uint32_t s = 0; s < numSets; s++) {
                array[s] = gm_malloc<uint32_t>(numWays);
        ...
        void replaced(uint32_t id) {
            assert(id < numLines);
            // array[set][way] = numWays - 1;
        }

        void manipulate(uint32_t id) {
            uint32_t set = id / numWays;
            uint32_t way = id % numWays;

            array[set][way] = MIN(static_cast<uint32_t>(array[set][way] * array[set][way]), numWays - 1);
        }
        ...
        template <typename C> inline uint32_t rank(const MemReq* req, C cands) {
            ...
            for (auto ci = cands.begin(); ci != cands.end(); ci.inc()) {
                ...
                if (!cc->isValid(id)) return id;

                uint32_t set = id / numWays;
                uint32_t way = id % numWays;
                uint32_t cnt = array[set][way];
            ...
            assert(bestId < numLines);
            assert(bestCnt == numWays - 1);
