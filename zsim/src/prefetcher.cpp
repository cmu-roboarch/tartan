        ...
        if (e.stride == stride) {
            ...
            if (e.conf.pred()) {  // do prefetches
                ...
                if (prefetchPos < 64 && !e.valid[prefetchPos]) {
                    MESIState state = I;
                    MemReq pfReq = {req.lineAddr + prefetchPos - pos, GETS, req.childId, &state, reqCycle, req.childLock, state, req.srcId, MemReq::PREFETCH, req.pc /*Kasraa: Prefetch requests carry PC of their trigger access*/};
...
uint64_t AnlPrefetcher::access(MemReq& req) {
    uint32_t origChildId = req.childId;
    req.childId = childId;

    uint64_t pc = req.pc;
    uint64_t region = req.lineAddr / (1024 / zinfo->lineSize);

    if (req.type == PUTS || req.type == PUTX) {
        mtable.terminateRegion(region);
    ...
    if (!mtable.isInArray(pc, region)) {
        mtable.allocate(pc, region);
        return parent->access(req);
    }
    ...
    if (pfBuffer.find(req.lineAddr) != pfBuffer.end()) {
        uint64_t storedCycle = pfBuffer[req.lineAddr];
        if (storedCycle < respCycle - 500 /*too old*/) {
            pfBuffer[req.lineAddr] = respCycle;
        }
        respCycle = MIN(respCycle, pfBuffer[req.lineAddr]);
    } else {
        for (uint64_t i = 0; i < mtable.generatePrefetches(pc, region); i++) {
            MESIState state = I;
            uint64_t pfAddr = req.lineAddr + i;
            MemReq pfReq = {pfAddr, GETS, req.childId, &state, reqCycle, req.childLock, state, req.srcId, MemReq::PREFETCH, pc};
            uint64_t pfRespCycle = parent->access(pfReq);

            pfBuffer[pfAddr] = pfRespCycle;
        }
    }
