 ...
 */
enum UopType : uint8_t {UOP_GENERAL, UOP_LOAD, UOP_STORE, UOP_STORE_ADDR, UOP_FENCE, UOP_OVEC};
...
#define MAX_UOPS_PER_INSTR 16  // technically, even full decoders produce 1-4 uops; we increase this for common microsequenced instructions (e.g. xchg).
...
class Decoder {
    ...
    public:
        static uint64_t lastOvecOperand;
        static std::queue<uint64_t> ovecLoadAddresses;
};
