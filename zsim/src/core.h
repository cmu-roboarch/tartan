...
class Core : public GlobAlloc {
    ...
    protected:
        g_string name;
        uint64_t lastOvecOperand;   // OVEC hack
    ...
    public:
        ...
        void setOvecOperand(uint64_t op) {
            ...
            // I'm the first to admit this is a bad programming. But remember
            // two things: never >1 threads emit OVEC, and never >1 OVECs fall
            // in one basic block in RoWild apps
            Decoder::lastOvecOperand = op;
        }
};
