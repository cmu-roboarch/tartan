...
SetAssocArray::SetAssocArray(uint32_t _numLines, uint32_t _assoc, ReplPolicy* _rp, HashFamily* _hf, bool _fcp) : rp(_rp), hf(_hf), numLines(_numLines), assoc(_assoc), fcp(_fcp) {
...
void SetAssocArray::postinsert(const Address lineAddr, const MemReq* req, uint32_t candidate) {
    ...
    // Manipulate recency counters
    if (this->fcp) {
        uint64_t numLinesInRegion = 1024 / zinfo->lineSize;
        uint64_t region = lineAddr / numLinesInRegion;
        uint32_t set = hf->hash(0, lineAddr) & setMask;
        uint32_t first = set*assoc;
        for (uint32_t id = first; id < first + assoc; id++) {
            if (array[id] / numLinesInRegion == region) {
                rp->manipulate(id);
            }
        }
    }
