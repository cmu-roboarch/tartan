...
BaseCache* BuildCacheBank(Config& config, const string& prefix, g_string& name, uint32_t bankSize, bool isTerminal, uint32_t domain) {
    ...
    if (arrayType == "SetAssoc" || arrayType == "SetAssocFcp") {
        numHashes = 1;
    } else if (arrayType == "Z") {
        numHashes = ways;
        assert(ways > 1);
    ...
    } else if (replType == "FCP") {
        rp = new FcpPolicy(numLines, candidates);
    ...
    if (arrayType == "SetAssoc") {
        array = new SetAssocArray(numLines, ways, rp, hf, false);
    } else if (arrayType == "SetAssocFcp") {
        array = new SetAssocArray(numLines, ways, rp, hf, true);
    } else if (arrayType == "Z") {
...
CacheGroup* BuildCacheGroup(Config& config, const string& name, bool isTerminal) {
    ...
    if (isPrefetcher) { //build a prefetcher group
        ...
        string type = config.get<const char*>(prefix + "type", "Stream");
        ...
        for (uint32_t i = 0; i < prefetchers; i++) {
            ...
            if (type == "Stream") {
                ...
                cg[i][0] = new AnlPrefetcher(pfName);
            }
        }
        return cgp;
    }
